...
Converse is a Firefox extension that indexes your Claude conversation history and skills
locally in your browser so you can search across the full text of every message and skill
file — not just titles. It also includes an optional Paste to File feature that converts
large text pastes into file attachments directly within the Claude input.
...
All conversation and skill data is stored exclusively in your browser's IndexedDB database on
your own device. This data never leaves your browser. It is never transmitted to the extension
author, any third party, or any server.
...
You can delete all stored conversation and skill data at any time using the **Clear all data**
option inside the extension drawer.
...
Skill content is fetched the same way, and only when you switch the drawer to Skills mode.
Each skill's archive is downloaded from the same `claude.ai` API and unpacked entirely inside
your browser — no external service is involved in reading it.
...
| `https://claude.ai/*` | Required to read your conversations and skills from the claude.ai API using your session |
